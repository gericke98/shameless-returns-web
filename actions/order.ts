...
export async function getOrder(prevState: any, formData: FormData) {
  ...
  if (rawFormData.order) {
    let ordernumber = rawFormData.order.toString();
    ...
    if (order) {
      if (rawFormData.email?.toString() === order.contact_email) {
        if (order.fulfillment_status === null) {
          // TO DO: HANDLE ORDER CHANGES BEFORE SHIPPING
          return {
            message:
        ...
        if (
          ...
          if (!orderDB && order.id) {
            // Check de si ya se había cambiado
            try {
              ...
              order.line_items.map(async (item: OrderLineItem) => {
                if (item.current_quantity > 0) {
                  ...
                  const priceWithDiscount =
                    Number(item.price) - item.discount_allocations[0].amount;
                  await db.insert(productsOrder).values({
                    ...
                    price:
                      priceWithDiscount.toString() || "No information provided",
