...
describe("valueBasket — order #311749", () => {
  ...
  it("charges only what was paid for a same-product swap, even at a different list price", () => {
    // F1: in the #311749 fixture both sizes list at the SAME price, so
    // lineRatio * listNow happens to equal the same-product short-circuit's
    // paid price. That coincidence must not be what proves the rule. Here
    // the target size lists HIGHER than the bought size, so this only
    // passes if the same-product short-circuit itself is firing.
    const items = [
      line({
        productId: "15296978026822",
        variant_id: "55598268973382",
    ...
    const catalogue = [
      product("15296978026822", [
        { id: "55598268973382", price: "55.00" }, // bought at 42.75
        { id: "55598268940614", price: "80.00" }, // target lists much higher
      ]),
    ];

    const basket = valueBasket(items, catalogue);

    expect(basket.exchangePrice).toBeCloseTo(42.75, 2);
    expect(basket.netAmount).toBeCloseTo(0, 2);
  });
  ...
  it("resolves a bare order-line id against the GID catalogue for a CROSS-PRODUCT exchange", () => {
    // F2: the same-product short-circuit compares GID to GID (productOf() is
    // keyed the same way on both sides in that path) and so cannot expose a
    // bare/GID mismatch. A cross-product line forces the price through
    // lineRatio(), which reads the ORIGINAL variant back out of the
    // catalogue by the line's BARE variant_id — that lookup is the one a
    // strict key silently breaks.
    const items = [
      line({
        productId: "15296978026822",
        variant_id: "55598268973382", // BARE — original variant, lists 55.00
        new_variant_id: vgid("54623384437062"), // different product, lists 69.00
        price: "27.50",
      }),
    ];
    ...
    // ratio = 27.50 / 55.00 = 0.5; 69.00 * 0.5 = 34.50
    expect(basket.exchangePrice).toBeCloseTo(34.5, 2);
    expect(basket.netAmount).toBeCloseTo(27.5 - 34.5, 2);
  });
});
